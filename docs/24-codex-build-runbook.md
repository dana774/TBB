...
8. **Deploy on Vercel** (the host). **UPDATE (2026-09-01):** the build already happened and diverged from
   this plan — the real app is **Astro** (not Next.js), lives in **`vgp-headless/`** on branch
   **`claude/new-session-q4or1d`**, serves **in-repo content** (`src/lib/content.ts`; **Wix CMS retired**),
   and uses **HubSpot** as CRM. In Vercel set **Root Directory = `vgp-headless`** and **Production Branch =
   `claude/new-session-q4or1d`**; the env var is **`HUBSPOT_TOKEN`** (+ optional `FIT_CALL_URL`, `BB_URL`) —
   **not** the old `WIX_*` trio. Review the **Preview URL** first; add `valugrowthpartners.com` under
   Vercel → Domains only at go-live. **Nothing goes to the live domain without Dana's explicit approval.**
   **The authoritative, corrected step-by-step is doc 25.**
