...
- Transcripts (35 episodes) intentionally NOT in these payloads (too large for staged JSON): export from the parked Wix site's Episodes collection (`transcript` field) via CMS export, convert to Shopify rich text, attach via `metaobjectUpdate` on the `transcript` field
